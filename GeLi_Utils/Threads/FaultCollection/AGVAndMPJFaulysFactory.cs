...
{
    public class AGVAndMPJFaulysFactory
    {
        ConcurrentDictionary<string, AGVAndMPJFaulysThread> taskDic =
            new ConcurrentDictionary<string, AGVAndMPJFaulysThread>();
       ...
       // WareHouseService wareHouseService = new WareHouseService();
        ...
        public void Start(double RepeatTime = 5000) //30秒执行一次
        {
            Control();
            //定时执行
            timer = new Timer();
        ...
        private void Timer1_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            {
                Logger.Default.Process(new Log(LevelType.Error, "AGVAndMPJFaulysThread:\r\n" + ex.ToString()));
                //Close();
                //Start(1000);
            }
        ...
        {
            //拿了所有仓库表
            List<MaPanJiInfo> list = maPanJiInfoDbBase.GetAll(true);
            ...
            foreach (MaPanJiInfo item in list)
            {
                ...
                {
                    AGVAndMPJFaulysThread aGVAndMPJFaulysThread = new AGVAndMPJFaulysThread(item);
                   // SameFloorRunThread sameFloorRunThread = new SameFloorRunThread(item);//当线程被实例化后已经开始循环了，此处传入仓库名
                    ...
                    Logger.Default.Process(new Log(LevelType.Info,
                    $"AGVAndMPJFaulysThread:{item.MpjName}开启码盘机故障记录执行线程。。。"));
                }
            ...
            foreach (string temp in taskDic.Keys)//对字段的名字循环   （保险）
            {
                ...
                {
                    AGVAndMPJFaulysThread aGVAndMPJFaulysThread = null;
                    taskDic.TryGetValue(temp, out aGVAndMPJFaulysThread);
                    if (aGVAndMPJFaulysThread.myTask != null)
                        aGVAndMPJFaulysThread.myTask.CloseTask();
                    taskDic.TryRemove(temp, out aGVAndMPJFaulysThread);
                }
            }
        }
